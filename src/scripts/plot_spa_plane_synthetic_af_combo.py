"""Two-panel SPA-plane figure, one base scorer per panel -- a variant of
...
Adequacy family (A) and Fluency family (F) with the single Adequacy-fluency
family (AF, src.lib.synthetic_scorers.base_scorer_family_additive_mean_af,
the paper's actual reported family): A = dial,
F = -dial, so the two aspects' effects move in OPPOSITE directions on one
dial instead of being swept independently.

Dial grid: AF itself uses AF_DIAL_GRID_SPARSE, a plotting-only half-
precision rebuild of src.lib.synthetic_scorers.AF_ADDITIVE_MEAN_DIAL_GRID
...
precision AF_ADDITIVE_MEAN_DIAL_GRID (T's own sweep wasn't asked to
change); J keeps plot_spa_plane_synthetic_combo.py's own sparse two-sided
grid unchanged.

AF is colored by a fixed red->blue diverging ramp (linearly interpolating
...
This also matches the project's existing convention (src.lib.spa_plane) of
blue=Adequacy, red=Fluency: AF's positive dial pushes toward Adequacy
(adds abar_k, subtracts fbar_k), so blue-at-+1 and red-at-1 are the same
color/aspect pairing used everywhere else in this codebase.

Usage: python -m src.scripts.plot_spa_plane_synthetic_af_combo
"""
...
from src.lib.spa_plane import build_af_seg_matrices, knowledge_lines, tradeoff_line
from src.lib.synthetic_scorers import AF_ADDITIVE_MEAN_DIAL_GRID, all_synthetic_family_points
...
# Sparse (20-point) rebuild of src.lib.synthetic_scorers.AFTJ_J_DIAL_GRID's own
...
N_DIAL_POINTS = 20
AFTJ_J_DIAL_GRID_SPARSE = tuple(round(-3.5 + 4.0 ** (k / (N_DIAL_POINTS - 1)), 10) for k in range(N_DIAL_POINTS))

# AF's own dial grid, plotting-only: AF_ADDITIVE_MEAN_DIAL_GRID's same
...
AF_DIAL_GRID_SPARSE = tuple(round(float(x), 2) for x in np.arange(-0.5, 0.5001, 0.04))

PANELS = [('a', 'hLEPOR', 'ende21'), ('b', 'MetricX-24-Hybrid', 'ende24')]

# AF's connecting line is dotted, like every family in the parent script;
# its marker is a diamond so it isn't confused with T's '+' or J's circle.
_STYLE = {
    'J': {'marker': 'o', 'linestyle': ':'},
    'T': {'marker': '+', 'linestyle': ':', 'cmap': 'Greens'},
    'AF': {'marker': 'v', 'linestyle': ':'},
}
...
_LEGEND_COLOR = {
    'AF': mcolors.LinearSegmentedColormap.from_list('af_rpb', [_FLUENCY_COLOR, _ADEQUACY_COLOR])(0.75),
...
# Fixed red(-1)->purple(0)->blue(+1) ramp for AF -- a straight 2-stop
...
_AF_CMAP = mcolors.LinearSegmentedColormap.from_list('af_rpb', [_FLUENCY_COLOR, _ADEQUACY_COLOR])


def _af_color(dial: float):
  """AF's color for one dial value: t = (dial + 1) / 2 against FIXED anchors
  dial=-1 (red) / dial=+1 (blue), not this sweep's own (smaller) actual
  range -- AF_DIAL_GRID_SPARSE only reaches +-0.5, so t only spans
...
if __name__ == '__main__':
  ...
  panel_data = {}
  for letter, base_scorer, dataset in PANELS:
    ...
    a_seg, f_seg = build_af_seg_matrices(dataset, systems, root=ROOT)
    if a_seg is None:
      sys.exit(f'{dataset}: too few jointly-valid adequacy/fluency segments')

    print(f'{dataset}/{base_scorer}: K={len(systems)} systems -- building sentinel lines', file=sys.stderr)
    tradeoff_pts = tradeoff_line(a_seg, f_seg)
    _, a_mean, _, f_mean = knowledge_lines(a_seg, f_seg)

    print(f'{dataset}/{base_scorer}: building synthetic families (AF + T + J)', file=sys.stderr)
    fam_am = all_synthetic_family_points(
        dataset, systems, root=ROOT, dial_grid=AF_ADDITIVE_MEAN_DIAL_GRID, synthesis='additive_mean',
        af_dial_grid=AF_DIAL_GRID_SPARSE).get(base_scorer)
    fam_off = all_synthetic_family_points(dataset, systems, root=ROOT, dial_grid=AFTJ_J_DIAL_GRID_SPARSE,
                                           synthesis='offset').get(base_scorer)
    if fam_am is None or fam_off is None:
      sys.exit(f'{dataset}: base_scorer {base_scorer!r} has no usable synthetic family')
    families = {'AF': fam_am['AF'], 'T': fam_am['T'], 'J': fam_off['J']}

    panel_data[letter] = dict(base_scorer=base_scorer, dataset=dataset, tradeoff=tradeoff_pts, a_mean=a_mean, f_mean=f_mean,
  ...
  for d in panel_data.values():
    all_xy += list(d['tradeoff']) + list(d['a_mean']) + list(d['f_mean'])
  ...
  for ax, (letter, _, _) in zip(axes, PANELS):
    ...
    for pts, color in ((d['tradeoff'], _TRADEOFF_COLOR), (d['a_mean'], _ADEQUACY_COLOR), (d['f_mean'], _FLUENCY_COLOR)):
      xs, ys = zip(*pts)
    ...
    for label in ('AF', 'T', 'J'):
      ...
      if label == 'AF':
        colors = [_af_color(dial) for dial, _ in dial_pts]
      elif label == 'T':
    ...
    ax.set_title(f'({letter}) {d["base_scorer"]} @ {d["dataset"]}', fontsize=TITLE_FONTSIZE)
  ...
  plot_path = os.path.join(ARTIFACTS_DIR, 'scorer_augmentation_families_hlepor-ende21_metricx24hybrid-ende24.pdf')
