...
Reuses src.lib.spa_plane's positional alignment (a_pos, f_pos, and a base scorer's own
...
# Same floor src.lib.consistency/src.lib.spa_plane use before trusting SPA's
# permutation p-values (kept as a local copy rather than importing the
# other modules' underscore-prefixed constants -- same convention src.lib.
...
# reordering needed, unlike src.lib.synthetic_scorer_preference.
...
def aspect_lookup_table(y: np.ndarray, aspect: np.ndarray) -> np.ndarray:
  ...
  small error-weight table, src.lib.mqm_scoring), the same discreteness plot_
...
# dial=0-reproduces-the-base scorer convention. Shared with src.lib.
...
def base_scorer_family_spa_points(
  ...
  two axes src.lib.spa_plane.scorer_spa_points uses. base_scorer_seg/a_pos/f_pos must
  ...
  (the official All-MQM total, src.lib.scorer_score_files.load_human_seg_scores,
  ...
  All MQM instead of either aspect (mirrors src.lib.synthetic_scorer_beta_grid.
...
def all_synthetic_family_points(
  ...
  positions with a_pos/f_pos -- the synthetic-scorer analogue of src.lib.
