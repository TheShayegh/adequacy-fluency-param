...
metric: re-run src.lib.consistency's pooled weighted-Kendall-tau
consistency measure, but with every dataset's systems reweighted to a
common target beta via the EXACT solver (src.lib.reweight_exact.
...
retired after cross-validation against src.lib.reweight_exhaustive showed
...
src.lib.reweight_exact has no such risk (a certified global optimum every
...
def solve_w_for_datasets(
  ...
  exhaustive-enumeration fallback, see src.lib.reweight_exact's module
