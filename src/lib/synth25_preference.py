...
allmqm_preference(SPA_synth25): src.lib.synthetic_scorer_preference's
dial-pair-preference method, applied to the SPA_synth25/PA_synth25
baseline (src.lib.synth25_metametrics, Shayegh et al. 2025's
...
Same dial families (src.lib.synthetic_scorers' A/B/T sweep for one base scorer
scorer), same reduction (src.lib.synthetic_scorer_preference.preference_score:
...
This is deliberately the one place system synthesis (src.lib.synth25_
metametrics) and scorer synthesis (src.lib.synthetic_scorers /
src.lib.synthetic_scorer_beta_grid, an unrelated axis -- see src.lib.synth25_
metametrics' own module docstring) are allowed to meet: src.lib.synth25_
...
from src.lib.synth25_metametrics import (
...
from src.lib.synthetic_scorer_preference import (
    NEG_J_DIAL_GRID, PREFERENCE_ADJACENT_ONLY_DEFAULT, dial_preset_suffix, preference_score, synthesis_suffix,
)
from src.lib.synthetic_scorers import DIAL_GRID, SYNTHESIS_GENERATORS, base_scorer_family_joint
from src.lib.utils import build_af_or_split_families, coverage_mask, load_base_scorer_coverage
...
def _dispatch_over_base_scorers(candidates, per_base_scorer_fn, progress=False, workers=1, job_args_fn=None, worker_fn=None):
  """Runs per_base_scorer_fn(base_scorer) for every candidate -- sequentially
  if workers<=1, or dispatched across a ProcessPoolExecutor (job_args_fn
  (base_scorer) building each job tuple, worker_fn the module-level
  picklable wrapper) when workers>1 -- the shared independent-per-base-
  scorer parallelization every dataset_synth25_preference* variant uses.
  progress=True prints one '[i/n] base_scorer status (elapsed, eta)' line
  per base scorer to stderr. Returns the list of non-None per-base-scorer
  results (candidate order under the sequential path; ProcessPoolExecutor.
  map's own completion order under the parallel path)."""
  t0 = time.time()
  results = []
  n = len(candidates)
  ...
  if workers > 1:
    jobs = [job_args_fn(base_scorer) for base_scorer in candidates]
    with cf.ProcessPoolExecutor(max_workers=workers) as ex:
      for i, (base_scorer, r) in enumerate(ex.map(worker_fn, jobs), 1):
        if r is not None:
          results.append(r)
        _log(i, base_scorer, r)
  else:
    for i, base_scorer in enumerate(candidates, 1):
      r = per_base_scorer_fn(base_scorer)
      if r is not None:
        results.append(r)
      _log(i, base_scorer, r)
  return results
...
def base_scorer_synth25_preference(
  ...
  `metametric_name` for one base scorer -- mirrors src.lib.synthetic_scorer_
  ...
  actual Adequacy-fluency family (src.lib.synthetic_scorers.
  ...
  src.lib.synthetic_scorer_beta_grid.base_scorer_beta_dial_grid's identically-
...
def dataset_synth25_preference(
  ...
  per_base_scorer = _dispatch_over_base_scorers(
      candidates, progress=progress,
      per_base_scorer_fn=lambda base_scorer: base_scorer_synth25_preference(
          ...
          adjacent_only=adjacent_only, use_af_family=use_af_family, af_dial_grid=af_dial_grid,
      ),
  )
...
def dataset_synth25_preference_all_pools(
  ...
  per_base_scorer = _dispatch_over_base_scorers(
      candidates, progress=progress, workers=workers,
      per_base_scorer_fn=lambda base_scorer: base_scorer_synth25_preference_all_pools(
          dataset, base_scorer, systems, root=root, metametric_name=metametric_name, dial_grid=dial_grid,
          synthesis=synthesis, tie_seed=tie_seed, num_permutations=num_permutations, seed=seed,
          adjacent_only=adjacent_only, use_af_family=use_af_family, af_dial_grid=af_dial_grid,
      ),
      job_args_fn=lambda base_scorer: (dataset, base_scorer, systems, root, metametric_name, dial_grid, synthesis,
                                        tie_seed, num_permutations, seed, adjacent_only, use_af_family, af_dial_grid),
      worker_fn=_base_scorer_synth25_job,
  )
...
def pool_beta_0s(dataset: str, systems: list[str], root: str = '.', tie_seed: int = DEFAULT_TIE_SEED) -> dict[str, float]:
  ...
  property of the pool's own system-level Adequacy/Fluency MQM (src.lib.
  ...
  mask = coverage_mask(a_pos, f_pos)
...
def preference_tag(
  ...
  src.lib.synthetic_scorer_preference.preference_tag's abbreviated suffix)
  ...
  (src.lib.synthetic_scorer_preference's synthesis_suffix/dial_preset_suffix,
  reused here directly); 'additive'/'additive_mean', 'geometric', and
  'extended' each get their own suffix so they never collide with an
  offset/linear cache for the same dataset/metametric."""
  suffix = synthesis_suffix(synthesis) + dial_preset_suffix(dial_preset)
...
def save_synth25_preference(
  ...
  them -- no beta/dial grid to store, unlike src.lib.synthetic_scorer_
...
def dataset_synth25_preference_J_all_pools(
  ...
  per_base_scorer = _dispatch_over_base_scorers(
      candidates, progress=progress, workers=workers,
      per_base_scorer_fn=lambda base_scorer: base_scorer_synth25_preference_J_all_pools(
          dataset, base_scorer, systems, root=root, metametric_name=metametric_name, dial_grid=dial_grid,
          tie_seed=tie_seed, num_permutations=num_permutations, seed=seed, adjacent_only=adjacent_only,
      ),
      job_args_fn=lambda base_scorer: (dataset, base_scorer, systems, root, metametric_name, dial_grid, tie_seed,
                                        num_permutations, seed, adjacent_only),
      worker_fn=_base_scorer_synth25_J_job,
  )
...
def pools_tag_J(dataset: str, metametric_name: str, dial_preset: str = 'neg') -> str:
  ...
  filenames valid; 'symmetric' (src.lib.synthetic_scorers.AFTJ_J_DIAL_GRID,
