...
the math for src/scripts/compute_beta_ess_preference_heatmap.py.
...
min-collision (max-ESS) weighting that realizes it exactly (src.lib.
...
RATIO OF TWO QUADRATIC FORMS in w. From src.lib.metametrics.
...
(src.lib.synthetic_scorers.base_scorer_family_additive_mean_af -- A = +dial,
F = -dial, so increasing dial trades fluency signal for adequacy signal),
reduced by src.lib.synthetic_scorer_preference.preference_score -- the fraction
...
Note this family is deliberately NOT routed through src.lib.synthetic_scorer_
...
from src.lib.metametrics import pairwise_p_values
from src.lib.scorer_score_files import load_human_seg_scores, load_scorer_seg_scores
from src.lib.spa_plane import DEFAULT_NUM_PERMUTATIONS, DEFAULT_SEED, positional_af_matrices
from src.lib.synthetic_scorers import AF_ADDITIVE_MEAN_DIAL_GRID, base_scorer_family_additive_mean_af
from src.lib.utils import coverage_mask
from src.lib.reweight_exhaustive import _binom_table, _simplex_lattice_chunk
...
# generator (see src/lib/reweight_exhaustive.py's docstring for the validation
...
def preference_scores(
  ...
  for start in range(0, n, chunk):
    ...
    for di, tbl in enumerate(tables):
      ...
      # errstate wraps the matmul for the same reason src.lib.metametrics.
...
def sample_simplex_beta_ess(
  ...
  while drawn < n_samples:
    ...
    if keep.any():
      ...
      # in src.lib.metametrics.pairwise_p_values.
      with np.errstate(all='ignore'):
        mu_a, mu_f = xk @ a, xk @ f
