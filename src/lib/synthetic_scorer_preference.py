...
(src.lib.synthetic_scorer_beta_grid.base_scorer_beta_dial_grid) over the full
...
family depends on synthesis (src.lib.synthetic_scorer_beta_grid.
base_scorer_beta_dial_grid):
  ...
  - synthesis='additive'/'additive_mean': T-family (dialed on All MQM,
    ...
    additive methods -- see src.lib.synthetic_scorers module docstring for why
    T isn't a meaningful construction under synthesis='offset'.
  - synthesis='offset': J-family (dialed on the JOINT (Adequacy, Fluency)
    pair, src.lib.synthetic_scorers.base_scorer_family_joint) gives
...
def full_range_betas(beta_lo: float, beta_hi: float, step: float, eps_frac: float = 1e-3) -> list[float]:
  ...
  inset by eps_frac*(hi-lo) at each end (src.lib.beta.build_beta_grid's own
...
def synthesis_suffix(synthesis: str) -> str:
  ...
  before this parameter existed valid; 'additive'/'additive_mean' (src.lib.
...
def dial_preset_suffix(dial_preset: str) -> str:
  """Shared by union_grid_tag/preference_tag: dial_preset='linear' (src.lib.
  synthetic_scorers.DIAL_GRID, the original 0/0.1/.../1 sweep) adds no
  suffix, keeping existing cache filenames valid; 'geometric' (src.lib.
  synthetic_scorers.GEOM_DIAL_GRID, {2^-i} packed toward dial=0) gets its
  own suffix -- '_geom'; 'extended' (src.lib.
  ...
  '_ext'; 'symmetric' (src.lib.synthetic_scorers.AFT_DIAL_GRID/AFTJ_J_DIAL_GRID,
...
def union_grid_tag(
  ...
  """Filename tag for src.lib.beta.union_beta_grid (the union of every
  ...
  return f'{dataset}_union_s{step:g}{suffix}{synthesis_suffix(synthesis)}{dial_preset_suffix(dial_preset)}'
...
def preference_tag(
  ...
  synthesis_suffix/dial_preset_suffix)."""
  suffix = '' if metametric == 'spa' else f'_{metametric}'
...
def resolve_tag(args, base: str) -> str:
  """The --tag/--union-grid/--aspect-base-scorers/--family-set cache-tag
  derivation compute_scorer_preference_vs_beta.py (writer) and
  plot_scorer_preference_vs_beta.py (reader) must agree on exactly, so the
  plot script can find the cache file the compute script wrote for the
  same flags: an explicit --tag always wins and is used as-is (no
  suffixes appended); otherwise union_grid_tag or preference_tag builds
  the base tag from the other flags, and _aspect_base_scorers/
  _{family_set} suffixes are appended only in that auto-derived case.
  `args` is the parsed argparse.Namespace from either script -- both
  define the same --n-steps/--step/--full-range/--metametric/--synthesis/
  --dial-preset/--union-grid/--aspect-base-scorers/--family-set/--tag
  flags, so one function serves both."""
  if args.tag:
    return args.tag
  if args.union_grid:
...
def save_preference_data(
  ...
  src.lib.synthetic_scorer_beta_grid.base_scorer_beta_dial_grid's
  ...
  (src.lib.reweight_exact.solve_w_exact solved once per beta). metametric
  records which weighted meta-metric ('spa' or 'pa') the families were
  scored with. synthesis records which src.lib.synthetic_scorers dial
...
# grid's range gets wide (src.lib.synthetic_scorers.EXTENDED_ADDITIVE_MEAN_
# DIAL_GRID, src.lib.synthetic_scorer_preference.NEG_J_DIAL_GRID -- both
...
def base_scorer_preference_by_beta(
  ...
  base scorer, from src.lib.synthetic_scorer_beta_grid.base_scorer_beta_dial_grid's
