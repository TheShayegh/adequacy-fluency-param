...
i.e. src.lib.metametrics.soft_pairwise_accuracy computed TWICE per scorer, once
...
functions:
  ...
  - build_af_seg_matrices: the sentinel lines only ever compare Adequacy MQM
    ...
    scorer is involved -- so they can use src.lib.mqm_scoring's own
    (doc, doc_id, seg_id)-keyed segment index directly.
  - positional_af_matrices: a scorer's own segment scores
    (src.lib.scorer_score_files.load_scorer_seg_scores) are indexed by LINE POSITION
    ...
    the sets where src.lib.mqm_scoring.OFFICIAL_RATINGS applies (or, for the
    ...
    sum of EVERY error -- src.lib.mqm_scoring's error_weight()/official-rating
    ...
    correlation against src.lib.scorer_score_files.load_human_seg_scores's OFFICIAL
...
from src.lib.mqm_scoring import load_segment_scores

# Same floor src.lib.consistency uses before trusting SPA's permutation
...
# datasets don't always hit an exact match, since src.lib.mqm_scoring
...
def positional_af_matrices(dataset: str, systems: list[str], root: str = '.'):
  ...
  (module docstring) -- the same index space src.lib.scorer_score_files' segment
...
def scorer_spa_points(
  ...
  scorer with full system-level coverage (src.lib.scorer_score_files.
  load_scorer_sys_scores -- same candidate gate src.lib.consistency.
