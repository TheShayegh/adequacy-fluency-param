...
from src.lib.metametrics import METAMETRICS, WEIGHTED_METAMETRICS, NEEDS_SEGMENT_SCORES, MetaEvalInput
from src.lib.scorer_score_files import (
    load_scorer_sys_scores, load_human_seg_scores, load_scorer_seg_scores,
    jointly_valid_columns,
)
from src.lib.wmt_metadata import is_reference_or_human, wmt_official_outliers, wmt_non_competative_outliers
...
# Fluency MQM: src.lib.mqm_scoring's raw-TSV seg_id for these is a per-document
...
# (src.lib.spa_plane.positional_af_matrices's self-check found 0.35 correlation
...
def real_systems(
  ...
  build it (e.g. src.lib.reweight_exact.solve_w_exact(a, f, beta) with a,f
  built in this same order).

  exclude_outliers (default True): THE PROJECT STANDARD as of this
  session -- src.lib.wmt_metadata.wmt_official_outliers is now the
  ...
  src.lib.spa_plane's scorer points) and would otherwise silently get a roster
...
def load_scorer_inputs(
  ...
  e.g. an beta grid, src.lib.reweighted_consistency -- can do it once and
...
def scorer_scores(
  ...
  under that weighting -- e.g. w = src.lib.reweight_exact.solve_w_exact(a,
...
def pool_weighted_tau(
  ...
  src.lib.reweighted_consistency (rankings under w(beta))."""
...
def weighted_consistency(
  ...
  `d` (see src.lib.reweighted_consistency.solve_w_for_datasets)."""
