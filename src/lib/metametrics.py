...
-> "1 - PCE") is a clean-room reimplementation of the algorithm in
...
system-weight vector w (e.g. from src.lib.reweight_exact.solve_w_exact)
instead of treating every system as equally important -- this is the
paper's weighted SPA (Eq. "wspa") and its analogues for the other four
meta-metrics, used to evaluate a scorer at a chosen adequacy-fluency
...
natural weighted generalization of its unweighted counterpart:
  ...
  - pearson/spearman: weighted covariance/variance (src.lib.beta's
    ...
    "pairwise-gap") -- so this is the same notion of "weighted" as
    everywhere else in the project, not a different one reused loosely.
    Spearman uses *weighted* ranks (a weighted-midrank transform) rather
    than plain ranks, so a near-zero-weight system can't anchor a rank
    position it barely occupies.
  - kendall/PA/SPA: inherently pairwise already (a sign or a p-value per
    ...
    used in src.lib.consistency (ties contribute 0 to the numerator but are
...
from src.lib.beta import weighted_mean, weighted_var
...
def weighted_kendall(x: MetaEvalInput, w: np.ndarray) -> float:
  ...
  denominator (see src.lib.consistency's identical convention, chosen there so
