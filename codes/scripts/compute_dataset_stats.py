"""Per-dataset descriptive statistics for the paper's Datasets paragraph
(material/paper.md S4.2), restricted to quantities the experiments actually
use, emitted as LaTeX tables.

Complements compute_alpha_table.py rather than replacing it: that script is
about the SYNTHESIZED pools (Shayegh et al. 2025's alpha_synth_* columns)
and the exhaustive per-subset-size alpha_0 sweep. This one is about the REAL
pool only, and adds the scorer axis, which compute_alpha_table.py never
touches.

NAMING: the paper's beta is this codebase's `alpha` (lib.alpha) throughout --
see lib/alpha.py. Columns are printed with the PAPER's names (beta_*),
computed by lib.alpha's alpha_* functions. The codebase's own `beta`
(lib.alpha.alpha_to_beta) is the paper's footnote reparameterization
beta_std and does not appear here at all.

Columns
-------
K          -- real systems, lib.consistency.real_systems (the project default
...
n_scorers  -- the BASE SCORER (donor) pool the augmentation experiments
              actually average over: lib.consistency.real_scorers with
              require_seg_scores=True, deduplicate=True,
              exclude_degenerate=True, further filtered by
              lib.synthetic_scorer_alpha_grid.donor_alpha_dial_grid's own
...
n_segments -- segments behind the system-level scores (post modal-coverage
              filter, mwb.mqm_scoring).

beta_0            -- natural (uniform-weight) balance.
beta_min/beta_max -- reachable range (Theorem 1), i.e. min/max pairwise beta_ij.
beta_range        -- beta_max - beta_min.

n_pairs, mean/median/std/iqr_beta_ij
           -- the C(K,2) pairwise-balance distribution (lib.alpha.
              pairwise_alphas). Theorem 3 makes any feasible target a convex
              combination of these, so their concentration is directly the
              reweighting's room to manoeuvre.
...
rho_af     -- Pearson correlation of a and f across systems: the rho of
              Theorem 5's certificate (condition iii), and the quantity the
              adequacy-fluency dial family needs strictly below 1 (S4.1).
...
Usage: python codes/scripts/compute_dataset_stats.py
Writes artifacts/dataset_stats.tex (all four LaTeX tables),
artifacts/dataset_scorers.md (the same name lists in Markdown), and prints
both to stdout.
"""
...
from lib.alpha import alpha_0, alpha_min_max, pairwise_alphas, variances
...
from lib.synthetic_scorers import _MIN_SPA_SEGMENTS
...
# all (its segment-level positional alignment cannot be validated --
# notes/data_inventory.md S4 -- so donor_alpha_dial_grid rejects EVERY
# donor), which already ruled it out of every augmentation experiment; it is
# now also dropped from the system-level-only leave-one-out table, so no
# result in the paper covers it. Kept as a named constant rather than
# silently omitted from ORDER so the reason survives.
EXCLUDED = ('enru22',)

# Paper-facing display names, in the paper's own S4.2 ordering.
DISPLAY = {
...
def donor_pool(dataset: str, systems: list[str], root: str = '.') -> list[str]:
  ...
  (lib/synthetic_scorer_alpha_grid.py) replicated exactly, minus the
...
def dataset_stats_row(a: np.ndarray, f: np.ndarray) -> dict:
  ...
  no I/O, so this is reusable per-subset the way lib.alpha_table.
  alpha_table_row is."""
...
full."""
  ...
  tex_path = os.path.join(ROOT, 'artifacts', 'dataset_stats.tex')
  with open(tex_path, 'w') as fh:
    fh.write(tex)

  md_path = os.path.join(ROOT, 'artifacts', 'dataset_scorers.md')
  with open(md_path, 'w') as fh:
    fh.write('# Base scorers (augmentation donors) per dataset\n\n')
    fh.write(
        'The pool every scorer-augmentation experiment averages over: '
        '`lib.consistency.real_scorers(require_seg_scores=True)` after '
        "`donor_alpha_dial_grid`'s coverage gate. See "
        '`codes/scripts/compute_dataset_stats.py`.\n\n')
    for name in ORDER:
