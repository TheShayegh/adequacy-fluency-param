"""For each dataset, report Kendall's tau-a (lib.consistency.pool_weighted_tau)
pooled across all leave-p-out (LpO) meta-evaluations of that dataset --
the tau analogue of compute_loo_icc.py's ICC(C,1): for a pool D of K real
systems (lib.consistency.real_systems), every size-p combination of
dropped systems gives one LpO subset D minus that combo (C(K,p) of them
total), each scored ONCE and treated as one RATER; scorers -- restricted
to lib.consistency.real_scorers(D), the project's canonical scorer screen
-- are the objects. Every pair of raters' scorer rankings is compared
(concordant/discordant counts, tau-a convention: ties drop from the
numerator but still count in the denominator), summed across ALL
C(n_raters,2) rater-pairs x scorer-pairs, and tau is computed ONCE from
those pooled totals -- the SAME pooling convention this project already
uses for cross-dataset scorer-ranking consistency (action_plan.md 6.5),
not a plain average of per-pair taus. p=1 (--leave-out 1, the default) is
plain leave-one-out (LOO).

NATURAL (uniform weight) by default -- unlike compute_loo_icc.py, this
does NOT also compute reweighted-to-alpha_0(D)/reweighted-to-median-
alpha_ij(D) conditions unless --reweighted is passed: "weighted" in
pool_weighted_tau refers to weighting each rater-pair's contribution to
the pooled tau by its own scorer-pair count (C(n_common,2)) -- an
intrinsic part of the tau-a pooling itself, nothing to do with
lib.reweight_exact's system-level alpha reweighting. Since this statistic
is about SCORER rankings (not system rankings), the project default is to
leave system-level reweighting out entirely (every LpO subset scored at
plain uniform weight) -- --reweighted is opt-in, for an explicit
comparison against compute_loo_icc.py's reweighted columns, not the
recommended way to run this script.

All the actual math/IO lives in lib.subsampling_stability.leave_p_out_tau
-- this script is just the per-dataset loop plus reporting. Unlike
leave_p_out_generalizability's icc_c1, pool_weighted_tau does NOT need a
fully-crossed matrix -- a scorer missing from one LpO subset still
contributes to every rater-pair it IS present in, rather than being
dropped globally.
...
STREAMED (project convention): header written immediately, each dataset's
row appended as soon as that dataset finishes, with per-dataset
elapsed/ETA progress on stderr.

Usage: python codes/scripts/compute_loo_tau.py
...
from lib.subsampling_stability import leave_p_out_tau
  ...
  with open(out_path, 'w') as f:
    if args.reweighted:
      ...
      f.write(f'Each row: C(K,{leave_out}) LpO subsets of the dataset (one per dropped '
              ...
              "as raters; Kendall's tau-a (lib.consistency.pool_weighted_tau) measures how "
      ...
      f.write(f'Each row: C(K,{leave_out}) LpO subsets of the dataset (one per dropped '
              ...
              "treated as one rater; Kendall's tau-a (lib.consistency.pool_weighted_tau) "
