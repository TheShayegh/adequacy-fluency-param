"""Computes Kendall's tau-a (lib.consistency.pool_weighted_tau, via
lib.subsampling_stability.leave_p_out_tau) pooled across a dataset's K
leave-one-out (LOO) raters, as a function of target balance alpha -- the
single-dataset, LOO-cross-rater analogue of the retired "type 1" pooled
cross-dataset consistency curve (codes/scripts/_obsolete_type1_type2_plot_
styles.py's render_type1_figure): same idea (a consistency statistic swept
over reweighted alpha, ESS-colored), but pooled across this ONE dataset's
K LOO subsets instead of across dataset pairs, and SPA-only (not one panel
per metametric).

For each alpha on the sweep, every one of the dataset's K LOO subsets
D\\{i} is reweighted (lib.reweight_exact.solve_w_exact) to that SAME
shared target alpha (identical "one target across every rater" convention
as leave_p_out_tau/leave_p_out_generalizability's own reweighted
condition), scored under SPA, and the resulting K scorer-rankings (fewer
if alpha was infeasible for some D\\{i}, Corollary 2) are pooled into one
tau via pool_weighted_tau -- exactly compute_loo_tau.py's --reweighted
tau computation, but swept over many alpha values instead of just
alpha_0(D)/median alpha_ij(D).

Alpha sweep: --n-alpha (default 21) points evenly spaced across the FULL
dataset D's reachable [alpha_min(D), alpha_max(D)] (lib.alpha.
alpha_min_max), inset by --eps-frac (default 1e-3) of the range at each
end -- lib.synthetic_scorer_orientation.full_range_alphas's own
...
Usage: python codes/scripts/compute_loo_tau_vs_alpha.py
...
from lib.alpha import alpha_0 as alpha0_of, alpha_min_max
