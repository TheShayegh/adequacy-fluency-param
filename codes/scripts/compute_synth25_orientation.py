...
PA_synth25 baseline (lib.synth25_orientation.dataset_synth25_orientation)
for one dataset and caches it (artifacts/data/synth25_orientation_<tag>.npz)
-- the expensive step (one SPA/PA permutation-test sweep per dial per
family per donor, lib.synth25_orientation.donor_synth25_orientation)
...
SPA_synth25/PA_synth25 (material/prior_work/metrics-dp-tradeoff.tex S2.4's
Row 7 -- real systems union adequacy-synthesized union fluency-synthesized)
take no alpha argument, so each family's orientation is a single dataset-
level number, not a curve.

--all-pools additionally (or instead, see below) computes the SAME
orientation for the other 5 pools lib.synth25_metametrics.POOL_BLOCKS
...
lib.synth25_metametrics.pool_alpha_0) -- written to a separate
artifacts/data/synth25_orientation_<tag>_pools.npz cache, for
...
Usage: python codes/scripts/compute_synth25_orientation.py [--dataset ende24]
...
keeps the historical lib.synthetic_scorer_orientation.NEG_J_DIAL_GRID
...
synthesis's own T. --dial-preset symmetric instead uses lib.
synthetic_scorers.ABTJ_J_DIAL_GRID -- the counterpart of --green-family
ABTJ (whose A/B/T side uses the DIFFERENT lib.synthetic_scorers.
...
from lib.synth25_orientation import (
...
from lib.synth25_metametrics import POOL_BLOCKS, SYNTH25_METAMETRICS
...
if __name__ == '__main__':
  ...
  parser.add_argument('--synthesis', choices=['offset', 'additive', 'additive_mean'], default='additive_mean',
                       help='which lib.synthetic_scorers dial family to build (see '
  ...
  parser.add_argument('--dial-preset', choices=['linear', 'geometric', 'extended', 'symmetric'], default='symmetric',
                       help="'linear': DIAL_GRID, 0/0.1/.../1. 'geometric': GEOM_DIAL_GRID, {2^-i} packed "
                            "toward dial=0. 'extended': lib.synthetic_scorers.EXTENDED_ADDITIVE_MEAN_DIAL_GRID "
                            "(additive_mean only, paired with donor_family_additive_mean's standardized=True "
                            "default). 'symmetric' (default, the committed setup): lib.synthetic_scorers."
