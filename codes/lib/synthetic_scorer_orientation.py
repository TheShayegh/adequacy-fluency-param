...
family depends on synthesis (lib.synthetic_scorer_alpha_grid.
donor_alpha_dial_grid):
  ...
  - synthesis='additive'/'additive_mean': T-family (dialed on All MQM,
    ...
    additive methods -- see lib.synthetic_scorers module docstring for why
    T isn't a meaningful construction under synthesis='offset'.
  - synthesis='offset': J-family (dialed on the JOINT (Adequacy, Fluency)
    pair, lib.synthetic_scorers.donor_family_joint) gives
...
def _synthesis_suffix(synthesis: str) -> str:
  ...
  before this parameter existed valid; 'additive'/'additive_mean' (lib.
...
def _dial_preset_suffix(dial_preset: str) -> str:
  """Shared by union_grid_tag/orientation_tag: dial_preset='linear' (lib.
  synthetic_scorers.DIAL_GRID, the original 0/0.1/.../1 sweep) adds no
  suffix, keeping existing cache filenames valid; 'geometric' (lib.
  ...
  filename convention for the same preset; 'extended' (lib.
  ...
  '_ext'; 'symmetric' (lib.synthetic_scorers.ABT_DIAL_GRID/ABTJ_J_DIAL_GRID,
...
def save_orientation_data(
  ...
  synthesis='offset') is expected, matching lib.synthetic_scorer_alpha_grid.
  ...
  (action_plan.md section 5) -- depends only on alpha, not on donor, since
  w*(alpha) is shared across every donor (lib.reweight_exact.solve_w_exact
  solved once per alpha). metametric records which weighted meta-metric
  ('spa' or 'pa') A/B/T/J were scored with. synthesis records which lib.
...
# grid's range gets wide (lib.synthetic_scorers.EXTENDED_ADDITIVE_MEAN_
# DIAL_GRID, lib.synthetic_scorer_orientation.NEG_J_DIAL_GRID -- both
...
def donor_orientation_by_alpha(
  ...
  depending on synthesis)} for one donor, from lib.synthetic_scorer_alpha_grid.
