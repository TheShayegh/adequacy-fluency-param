...
the math for codes/scripts/compute_beta_ess_preference_heatmap.py.
...
min-collision (max-ESS) weighting that realizes it exactly (lib.
...
RATIO OF TWO QUADRATIC FORMS in w. From lib.metametrics.
...
(lib.synthetic_scorers.donor_family_additive_mean_diagonal -- A = +dial,
B = -dial, so increasing dial trades fluency signal for adequacy signal),
reduced by lib.synthetic_scorer_orientation.orientation_score -- the fraction
...
Note this family is deliberately NOT routed through lib.synthetic_scorer_
...
from lib.reweight_exhaustive import _binom_table, _simplex_lattice_chunk
...
# generator (see lib/reweight_exhaustive.py's docstring for the validation
...
def preference_scores(
  ...
  for start in range(0, n, chunk):
    ...
    for di, tbl in enumerate(tables):
      ...
      # errstate wraps the matmul for the same reason lib.metametrics.
...
def sample_simplex_beta_ess(
  ...
  while drawn < n_samples:
    ...
    if keep.any():
      ...
      # in lib.metametrics.pairwise_p_values.
      with np.errstate(all='ignore'):
        mu_a, mu_f = xk @ a, xk @ f
