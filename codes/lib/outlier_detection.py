...
its own docstring below. It is baked directly into lib.consistency.
...
assume every one of the 15 is non-empty (see the many `if not systems:
continue`-style guards added across codes/scripts/ this session, and
lib.consistency.real_systems' own docstring for exactly what "default"
...
& Hoya 1993 for the underlying MAD scale) -- see artifacts/inventory_
outliers/README.md for the exact math and citations. Iterated the same
masking-resistant way as the single-aspect version below, at GK_THRESHOLD
(Z > 4). It is NOT applied automatically anywhere -- unlike wmt_official,
...
lib.outliers.mad_outliers' modified z-score, never the joint relationship
...
given how little they shift alpha_0), which neither outlier detector above
has any way to see, since neither looks at robustness/tau at all.
...
from lib.outliers import _MZ_CONSTANT, DEFAULT_THRESHOLD, mad_outliers
...
def iterative_single_aspect_outliers_either(
  ...
  action_plan.md 6.5's outlier screen literally says "on each aspect,"
  which every caller so far (iterative_single_aspect_outliers itself, and every
  script that uses it) has only ever applied to adequacy alone. This is
  the two-aspect version: each pass recomputes modified z-scores for BOTH
...
def gk_robust_loc_cov(x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
  ...
  univariate modified z-score (lib.outliers) already uses -- unlike a
...
def iterative_gk_outliers(
  ...
  gk_robust_loc_cov -- Gnanadesikan & Kettenring 1972), not by either
  aspect alone. Default threshold is GK_THRESHOLD (Z > 4), not the
  single-aspect DEFAULT_THRESHOLD (3.5) -- see artifacts/inventory_
  outliers/README.md for the exact math and the threshold rationale.
  ...
  detector is meant to avoid) -- see artifacts/inventory_outliers/README.md
...
def wmt_official_outliers(dataset: str, systems: list[str] | None = None) -> set[str]:
  ...
  lib.consistency.real_systems() (exclude_outliers=True, its default), so
