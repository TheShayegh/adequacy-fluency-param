...
experiments (action_plan.md sections 1.1, 4, 6.5).
...
lib.systems.join_key. Metric-scores rosters are also wider than our
MQM-rated roster (WMT's full submission list vs the MQM-annotated subset,
same pattern noted throughout notes/data_inventory.md); we keep only the
intersection.
"""
...
from lib.systems import is_reference_or_human, join_key
...
def discover_metrics(dataset: str, root: str = '.') -> dict[str, str]:
  ...
  every metric with a .sys.score file in this dataset (lib.metric_names.
...
def _load_score_file(path: str) -> dict[str, list[float]]:
  ...
  are the one exception -- space-delimited, not tab (same quirk
  compare_official.py's load_official() already works around for the
  .sys.score sibling files) -- so fall back to splitting on the last space
  when no tab is present."""
