"""SPA(scorer; alpha) vs. target balance alpha (see codes/scripts/compute_
spa_vs_alpha.py): shared math (the ESS>=target_ess sweep-range solve) and the
.npz cache format, so the compute script (expensive: solve_w_exact per alpha,
a permutation test per scorer) and any number of plot scripts (cheap: just
matplotlib) can be iterated on independently.
"""
...
def _find_ess_boundary(a_D: np.ndarray, b_D: np.ndarray, lo: float, hi: float, target_ess: float) -> float:
  ...
  boundary, action_plan.md section 5)."""
  def f(alpha):
