...
weight table and whose Table 5 we validate against directly in
compare_wmt24_paper.py).
...
compare_official.py): exact to 4 decimal places for most sets, small
explained residuals (a document needing multi-rater data our single-rater
default TSV lacks; a couple of raw-data naming quirks) for the rest.
...
def classify_aspect(category: str) -> str | None:
  ...
  taxonomy) before any check: generalMT2022/enzh uses a third variant that
  mixes conventions, e.g. "locale_convention/name" (underscore before the
  slash) and "non_translation" -- found via extract_labels.py, which
  flagged both as silently falling through to None (real weight, dropped
  from both a and b) before this normalization was added.
  """
  if not category:
    return None
...
def filter_modal_coverage(per_segment: pd.DataFrame, min_fraction: float = 0.5) -> pd.DataFrame:
  ...
  against official scores, see compare_official.py) to incorrectly drop
  individual, legitimately-rated segments in the official-ratings path
  whenever coverage varies by a little for reasons unrelated to the
  disjoint-sub-sample problem (e.g. one unrelated system's rating happens
  to be missing at a position: ted_ende/wmt21.tedtalks has 13 segments at
  13/14 = 93% coverage, which must stay). 50% cleanly separates both
  observed cases; it's a no-op on the many sets that are already fully
  rectangular.
  """
...
def per_segment_scores(seg_df: pd.DataFrame) -> pd.DataFrame:
  ...
  synthesized-pool construction (per-segment cross-system ranking), so both
  operate on the identical, already-filtered set of (system, segment) cells.
  """
...
OFFICIAL_RATINGS = {
    ...
    # (already an exact match against the official total, see
    # compare_official.py -- the weighting is validated regardless).
}
...
# All 16 system-sets in wmt-mqm-human-evaluation. The first 5 are the exact
# sets used by Shayegh et al. (2025); the rest are bonus real system-sets
# (2020-2022, TED, He-En) useful for the subset-consistency / cross-year
# experiment (action plan, section 6.5) precisely because they weren't
# cherry-picked for this project.
SETS = {
...
# generalMT2022/enzh is excluded: its raw TSV has a different/broken column
# layout (see notes/data_inventory.md), and no official rating data exists
# for it either.
...
def load_segment_scores(set_name: str, root: str = '.') -> pd.DataFrame:
  ...
  to per-system means. Used for per-segment cross-system ranking (the
  synthesized-pseudo-system construction of action_plan.md section 2.1)."""
