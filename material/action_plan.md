...
**What synthesis is still good for.** None of this makes synthesis useless. Retargeted through our own balance definition (§3.2) rather than the $B$ indicator, it becomes a legitimate **range-extender** in the mechanical experiments only, with the SPA caveat owned, showing the knob keeps turning and the solver stays cheap outside the natural range (§6.2, §6.6).
...
Division of labor: the **synthetic-scorer testbed** supplies ground truth for *internal validity*; the **removal-robustness experiment** supplies *external value* on real data with no ground truth; **synthesis** appears as a critique target (§2) and as a **range-extender** in the mechanical experiments only (§6.2, §6.6).
...
### 6.5 Removal robustness (real data, no ground truth — the strongest external experiment)

The only real-data experiment needing **no ground truth**: robustness under system removal is measurable without knowing the right answer, and it operationalizes the thesis of Shayegh et al. (2025) more directly and more sharply than a resampling design — if the meta-evaluation verdict is at the mercy of system selection, then removing a handful of real systems should visibly move the traditional (unweighted) scorer ranking, and recalibrating the larger pool to the removed-system pool's *own* natural balance should measurably close that gap.

**Design.** Fix a real system pool $D$ (after the outlier screen below), $|D|=K$, and a level $L=\Delta K\ge1$. For **every** $d\subset D$ with $|d|=L$ — all $\binom{K}{L}$ of them, not a random sample — let $D' = D\setminus d$. Write $R(\cdot)$ for the scorer-level meta-evaluation ranking (per-scorer meta-metric scores, e.g. SPA) computed with **uniform** system weights, the traditional design. The **before** comparison is the traditional design's exposure to exactly this removal:
$$\tau_{\text{before}}(D,D') = \tau\big(R(D),\,R(D')\big),$$
Kendall's $\tau$ (tie-aware tau-a: concordant $-$ discordant, over all scorer pairs) between the scorer ranking on the full pool and the scorer ranking after $d$ is dropped — exactly how much the traditional verdict would have shifted had these $L$ systems never been submitted.

**Calibration.** Reweight $D$ — using the exact solver of §3.11, with $D$ playing the role of the full system set — to hit $D'$'s own **natural balance** $\alpha_0(D')$ (§3.10), giving weights $w^\star_D(\alpha_0(D'))$ and a calibrated ranking $R\big(D;\,w^\star_D(\alpha_0(D'))\big)$. The **after** comparison is
$$\tau_{\text{after}}(D,D') = \tau\Big(R\big(D;\,w^\star_D(\alpha_0(D'))\big),\,R(D')\Big),$$
and **removal robustness** is $\tau$ itself (not its complement); the object of interest is the improvement
$$\Delta\tau(D,D') \;=\; \tau_{\text{after}}(D,D') \;-\; \tau_{\text{before}}(D,D').$$
$\Delta\tau>0$ says: pinning $D$'s balance to what the smaller, post-removal pool would have realized on its own brings the big-pool verdict closer to the verdict you would actually get after the removal — calibration undoes some of the instability the traditional design suffers.

**Which direction to reweight, and why.** We reweight the *larger*, more flexible pool $D$ to match the *smaller* pool $D'$'s own natural balance — not the reverse (reweighting $D'$ to $D$'s natural $\alpha_0$). This is not a coin flip between two symmetric options: $D'$ is not independent data, it is $D$ with $d$'s evaluations **deleted**, so the two directions differ in kind, not just degree. Reweighting $D$ never leaves $D$'s information set — all $K$ systems' scores stay in the ranking computation, only reweighted — and the literal "drop $d$" point (weight $0$ on $d$, uniform on the rest) is itself one feasible point of $D$'s own simplex that already realizes $\alpha_0(D')$ exactly; the calibration search for the point closest to uniform at that target is searching a space that already contains a configuration behaviorally identical to natural $D'$. Reweighting $D'$ instead asks something structurally different: $D'$ has *lost* $d$ entirely, not down-weighted it, and no reweighting of the remaining $K-L$ systems can reconstruct the ranking information $d$ carried — matching one scalar ($\alpha$) cannot recover it. The asymmetry is also visible in the machinery already established: since $D'\subset D$, $D$'s pairwise balances are a superset of $D'$'s, so $D$'s reachable range always **contains** $D'$'s narrower one (Corollary 2) and $D$'s ESS ceiling ($K$) always exceeds $D'$'s ($K-L$, Lemma 2) — $D'$ is the more constrained side on both feasibility margin and achievable diversity, which is what makes a reversed calibration degrade even where it remains technically feasible, not only where it fails outright (concretely observed as an outright blow-up with MSLC in ende24, before the outlier screen below removed it from the pool by default).

**This is a falsifiable prediction, not just a construction choice, and we tested it (ablation).** If calibration helps only because $D$ retains full information and reweighting searches a space containing the true target, reversing the direction should not just risk infeasibility — it should make robustness measurably *worse*, and increasingly so as more of $d$'s information goes missing. We ran both alternatives at levels $1$-$1$, $2$-$2$, $3$-$3$ across all six real-word datasets: pooled $\Delta\tau$ for the reversed direction is negative in 17 of 18 cells and grows more negative with $L$ (opposite of forward's positive, growing trend); reweighting **both** sides to the mean of the two natural $\alpha$'s lands in between, mostly negative but smaller in magnitude — consistent with mixing a real gain (forward) and a real loss (reversed) rather than capturing either mechanism. No infeasibility occurred at these levels, confirming the outlier screen already keeps the pathological case out of the pool. The direction choice is therefore not an arbitrary implementation detail: it is the one predicted to work by the information asymmetry, and it is the only one of the three that does.

**Same-pair design — the gap is the improvement, and it needs no resampling.** $\tau_{\text{before}}$ and $\tau_{\text{after}}$ are computed on the **identical** $(D,D')$ pair, so every uncontrolled axis besides balance ($\rho$, composition, whatever else moves the scorer ranking) sits in both terms and cancels in $\Delta\tau$ — the same cancellation logic as a same-subset design, but costing nothing extra here: because we enumerate every $d$ rather than drawing subsets at random, the result carries no sampling noise from the draw itself, only genuine pool-to-pool variation.

**Pooling.** Report one number per (dataset, level), **not** a per-$d$ average: sum concordant/discordant/tie counts for $\tau_{\text{before}}$ and for $\tau_{\text{after}}$ across all $\binom{K}{L}$ removals first, then take $\tau$ of each total — matching the pooling convention used elsewhere (§6.1, §6.6) rather than averaging $\binom{K}{L}$ noisy per-removal $\tau$ values, several of which are near-degenerate at small scorer counts. This pooled $\Delta\tau$ is the headline statistic.

**Outlier screen.** Before building $D$, iteratively remove extreme systems by modified z-score (Iglewicz & Hoya 1993, $|z|>3.5$) on each aspect, repeating on the shrinking pool until a pass flags nothing — masking-resistant, since one extreme system (e.g. MSLC) can otherwise dilute the median/MAD enough to hide a second one. This is the default; a system's contribution to $\Delta\tau$ should reflect ordinary selection variability, not a single extreme submission. A second, optional screen flags **leverage points** — systems whose $(\Delta\alpha_0,\ \tau_{\text{before}})$ pair is a studentized-residual outlier (Bonferroni-corrected) in a cheap level-1 diagnostic pass — available for robustness checks but off by default in the headline numbers, so those report the full real pool modulo only the extreme-outlier screen.

**The headline figure.** x-axis $\Delta K = L$; y-axis pooled $\Delta\tau$; one bar-chart panel per real dataset. Run at $L=1,\dots,5$ ("$L$-$L$": no staged context, $D$ is always the full screened pool) across all six 2023+2024 datasets (ende23, zhen23, heen23, ende24, enes24, jazh24), pooled $\Delta\tau$ is **positive and increasing in $\Delta K$ in every one of the 30 (dataset, level) cells**: removing more systems together both destabilizes the traditional verdict more and gives calibration more room to help, and the two effects compound. The size of the effect is dataset-dependent — steep in ende23 and jazh24 (pooled $\Delta\tau$ growing roughly $5$–$6\times$ from $L=1$ to $L=5$), much flatter in zhen23 and enes24 — which is itself informative about how selection-sensitive each dataset's traditional verdict already is.

**Staged removal (generalization, not the headline).** The same machinery generalizes to a **level $A\text{-}B$** notation: $A$ is the total number of systems ultimately removed, $B\le A$ the number removed together as the final step (the "$L$-$L$" design above is $B=A$, i.e. no staged context). $A\text{-}1$ recovers single-system removal from an already-reduced context of size $A-1$. This lets a future pass test whether the robustness gain is path-dependent — does it matter *which* systems were already gone before the final $B$ are dropped, not only how many are dropped at once — but the headline claim above uses only the context-free $B=A$ diagonal.
...
**Recommendation.** Report all five meta-metrics on equal footing ($M(\alpha)$ curves, removal robustness, testbed), reading each through the key of §4. The discordant-pair target is the PA-faithful *robustness alternative*. Shayegh et al. (2025) already found PA and SPA broadly consistent, so we expect agreement here; their discordant fraction (Table 5: 43–71%) is descriptive, not load-bearing.
