...
\author{Behzad Shayegh$^\star$ \\\And
  Niloofar Kazemi \\}
...
% \footnotetext{Partially supported by RBC Borealis.}
\footnotetext{Correspondence to: \href{mailto:the.shayegh@gmail.com}{the.shayegh@gmail.com}}
\endgroup
\begin{abstract}
Machine translation meta-evaluation inherits the adequacy–fluency tradeoff, and its balance is largely determined by the combination of translation systems in the data. This system set is a small, filtered sample whose characteristics change heavily across years and language pairs; it does not represent the true system distribution. Consequently, the adequacy–fluency balance is often unrepresentative and subject to change. For sensitive domains, controlling this balance is critical. We expose this balance as a tunable control parameter, $\beta$. To achieve a target $\beta$, we reweight existing systems while minimizing distortion from uniform weighting, ensuring the evaluated systems remain real and representative. We provide an exact optimization algorithm with theoretical guarantees and pruning mechanisms to compute these weights. To validate meta-evaluation faithfulness, we design a scorer-augmentation framework that establishes a ground-truth relative identity for real metrics. Results demonstrate our reweighting method effectively controls the adequacy–fluency balance and preserves faithfulness, outperforming prior approaches. Finally, we analyze the performance of popular scorers across this parameter.
\end{abstract}

\section{Introduction}

Machine translation (MT) is typically judged by automatic metrics, which we refer to as \emph{scorers}.\footnote{We use the term ``scorer'' instead of ``metric'' to avoid confusion with meta-metrics.} Deciding which scorer to trust is itself an evaluation problem known as \emph{meta-evaluation}~\citep{callison-burch-etal-2007-meta}. Concretely, meta-evaluation fixes a set of translation systems and scores them once with a human protocol taken as the ground truth~\citep[Multidimensional Quality Metrics, MQM;][]{freitag-etal-2021-experts}, and once with each candidate scorer. It then asks how well the scorer's ranking of the systems agrees with the human ranking, as measured by a \emph{meta-metric} such as pairwise accuracy~\citep[PA;][]{kocmi-etal-2021-ship} or soft pairwise accuracy~\citep[SPA;][]{thompson-etal-2024-improving}. The WMT~metrics shared task \citep{freitag-etal-2023-results} performs this meta-evaluation every year, and it sits at the top of the MT stack: it identifies the best scorers, which subsequent research adopts and reports, in turn shaping the next generation of MT systems. Both the scorers and the MT systems therefore silently inherit whatever bias the meta-evaluation imposes.

The MQM ground truth is a multi-component quantity. A translation is penalized both for {adequacy} errors (failing to convey the meaning of the source) and for {fluency} errors (being ungrammatical or unnatural), and its overall score is the sum of the two~\citep{freitag-etal-2021-experts}. \citet{flamich2025feedbirdsscoreaccuracynaturalness} show that these two aspects trade off against each other, and \citet{shayegh-etal-2025-feeding} show that the tradeoff reaches the {meta-evaluation} level: the overall human ranking is dominated by whichever aspect varies more across the systems being compared, so the scorers judged best are those aligned with that aspect. Crucially, this lean is heavily driven by {system selection}~\citep{shayegh-etal-2025-feeding}.
...
This is especially concerning because the included systems are a small, filtered subset, and thus likely a biased sample of the true, unknown distribution. Figure~\ref{fig:1} shows that the lean differs substantially across WMT datasets. Consequently, meta-evaluation produces uncontrolled scorer rankings, risking the endorsement of undesired scorers that misdirect future model development. This motivates the need for an explicit control mechanism.

\citet{shayegh-etal-2025-feeding} attempt to address this problem through the lens of ``{debiasing}.'' They frame system-selection imbalance as undesirable {extrinsic bias} and introduce a system-synthesis method to reduce it. However, not all system-selection imbalance is undesirable; only its deviation from the true system distribution is. As the true distribution remains unknown, this attribute is unmeasurable, leading us to abandon debiasing in favor of control. Furthermore, we show that the system synthesis produces unrepresentative systems, yielding an unfaithful meta-evaluation.


In this work, we expose the adequacy--fluency balance as a single {control parameter} $\beta$, transforming a previously uncontrolled bias into a deliberate, tunable choice for domain experts. We treat the systems in MQM data as a biased sample and {reweight} them to meet $\beta$ while minimizing distortion from uniform weighting. Reweighting is applied at the system level; systems remain real and representative, preserving the faithfulness of the meta-evaluation. It is an instance of importance sampling~\citep{tokdar2010importance} with minimal distortion of weights~\citep{deville-sarndal-1992}.

The reweighting is a nontrivial optimization problem, for which we design an exact algorithm, together with pruning mechanisms that improve its efficiency and certificates that verify optimality. We prove the soundness and completeness of the algorithm, its pruning mechanisms, and its certificates, and provide an empirical computational-cost analysis.

Validating meta-evaluation is challenging without a ground truth. To address this, we introduce a scorer-augmentation framework that injects controlled signals into real base scorers, preserving the inherent statistical signatures of real metrics while providing known \emph{relative identity}. Using these augmented scorers, we assess a meta-evaluation's (1)~adequacy-over-fluency preference, (2)~MQM adherence, and (3)~preference for explainability by MQM. The latter measures noise avoidance while preserving the natural scorer--MQM relationship. We demonstrate that our reweighting method effectively controls the adequacy--fluency balance and preserves faithfulness, outperforming \citet{shayegh-etal-2025-feeding}'s synthesis method.

Finally, we report and analyze the meta-evaluation scores of popular scorers, including different variants of xCOMET~\citep{10.1162/tacl_a_00683} and MetricX~\citep{juraska-etal-2024-metricx}, across the exposed adequacy--fluency balance parameter.

In summary, we: (1) expose the adequacy--fluency imbalance of MT meta-evaluation as a free parameter; (2) provide an exact algorithm that controls this parameter by importance-weighting the systems in the meta-evaluation, minimizing the distortion from uniform weighting so as to maximally preserve the meta-evaluation's other characteristics; and
(3) propose a controlled scorer-augmentation method to assess a meta-evaluation's adequacy-over-fluency preference, MQM adherence, and  preference for explainability by MQM.

\section{Background and Related Work}

\subsection{Meta-Evaluation}

A translation evaluation scorer is evaluated through meta-evaluation, which measures its agreement with human annotations across a fixed pool of translation systems \citep{callison-burch-etal-2007-meta}. Expert error annotation under Multidimensional Quality Metrics~\citep[MQM;][]{mariana2015multidimensional} has been established as the standard ground truth for the WMT metrics shared task~\citep{freitag-etal-2021-experts, freitag-etal-2023-results, kocmi-federmann-2023-large, kocmi-etal-2025-findings-wmt25}. Our work focuses on MQM-based meta-evaluation following recent standards.

\paragraph{Meta-metrics.}
While system-level agreement was traditionally measured via Pearson~\cite{machacek-bojar-2013-results, machacek-bojar-2014-results}, \citet{mathur-etal-2020-tangled} demonstrate that correlation is highly sensitive to outlier systems in the evaluation pool. To address this, \citet{kocmi-etal-2021-ship} introduce pairwise accuracy~(PA), which measures the fraction of system pairs whose human ordering is correctly reproduced. To overcome PA's coarse resolution and tie-handling limitations at the segment level \citep{deutsch-etal-2023-ties}, \citet{thompson-etal-2024-improving} develop soft pairwise accuracy~(SPA), which weights system pairs by the statistical significance of both human and scorer gaps. SPA serves as the official system-level meta-metric of WMT24~\citep{freitag-etal-2024-llms}. We follow the line of research and focus on SPA as our main metric. We additionally report results for Pearson correlation in Appendix~\ref{app:pearson}.
...
The distinction between translation adequacy and fluency originates in early evaluation campaigns~\citep{white-etal-1994-arpa} before being largely collapsed into error-weighted penalties \citep{lommel-etal-2014-multidimensional}. Information-theoretic analyses prove that accuracy and naturalness genuinely trade off, making a single scalar summary insufficient \citep{flamich2025feedbirdsscoreaccuracynaturalness}. Moreover, downstream user trust responds to fluency independently of adequacy \citep{martindale-carpuat-2018-fluency}.
\citet{shayegh-etal-2025-feeding} demonstrate that this trade-off extends to the meta-evaluation level: Meta-evaluation inherits the pool's lean, favoring adequacy-oriented scorers in recent WMT MQM datasets due to skewed system selection.
...
\end{align}
In other words, $\beta$ is the balance of mean-squared pairwise gaps, and controlling $\beta$ controls the real driver of the meta-evaluation's lean.
...
I_2{+}2\eta
\begin{pmatrix}
\hat\beta\|\tilde f\|^2{-}(1{-}\hat\beta)\rho^2\|\tilde a\|^2 ,& -(1{-}\hat\beta) \rho\sqrt{1{-}\rho^2} \|\tilde a\|^2 \\[5pt]
-(1{-}\hat\beta)\rho\sqrt{1{-}\rho^2}\|\tilde a\|^2         ,& -(1{-}\hat\beta)(1{-}\rho^2)\|\tilde a\|^2 \end{pmatrix}{\succeq}0 \tag{iii}}
\end{align}
...
each meta-evaluation's adequacy-over-fluency preference, MQM adherence, and preference for explainability by MQM. To this end, we first develop a controlled
testbed that reveals ground truth for these three aspects. Then, we benchmark our reweighting
...
the fluency of the translation), which is not available. To obtain such a
ground truth, we propose to augment scorers from a \emph{base scorer} in a
controlled way, so that the augmented scorers' identities relative to
one another are known by construction, even though no scorer's absolute
identity is. We use this relative identity as ground truth for evaluation
...
\end{compactitem}
An ideal meta-evaluation should typically prefer an augmented scorer that better adheres MQM or prefers explainability by MQM.
...
unchanged avoids confounding a family's preference signal with a change
in SPA's confidence signal.
...
We include every scorer in the MQM datasets as a base scorer. This
includes the competitive scorers as well as the diagnostic ones
included by WMT. For each meta-evaluation and each dataset, we report the
average scores across all base scorers.

\paragraph{Dial grid.} The closer two $d$ values are, the harder it is for
a meta-evaluation to recognize their relative identity. We empirically choose $d$ grid for the best readability of results. For MQM adherence, $d$ ranges from $-0.5\rightarrow1.5$ and for explainability by MQM, from $-2.5\rightarrow0.5$, both with an
exponentially growing step size. For adequacy-over-fluency preference, $d$ ranges linearly from $-0.5\leftrightarrow+0.5$.

\paragraph{Datasets.} We use all MQM data published by~\citet{freitag-etal-2021-experts}.\footnote{\href{https://github.com/google/wmt-mqm-human-evaluation}{github.com/google/wmt-mqm-human-evaluation}} We restrict our analysis to datasets from 2021 onward due to limited standardization and support for older datasets. We specifically exclude ende~23 because many of its MQM annotations are categorized as ``other'' rather than being properly assigned to adequacy or fluency categories. This leaves 11 datasets: ende~24, enes~24, jazh~24, heen~23, zhen~23, ende~22, zhen~22, ende~21, zhen~21, ted~ende, and ted~zhen. We include all available scorers, excluding only those without segment-level scores (e.g., corpus-level chrF), and deduplicating perfect duplicated scorer pairs. We focus exclusively on automatic metrics, omitting human references. Across all datasets, we evaluate all available systems except for M2M100\_1.2B-B4 and MSLC, which are officially flagged for exclusion.
...
\end{center}
\vspace{-5pt}
\caption{Main results for ende21 (left) and ende24 (right). Curve color intensity represents the estimated support size (ESS); a lower ESS indicates fewer actively involved systems, reducing reliability. The strictly increasing adequacy-over-fluency curve demonstrates that $\beta$ directly controls this balance. The natural balance $\beta_0$ for each dataset is marked by a vertical dotted line, with its intersections indicating the original meta-evaluation's performance. Higher MQM adherence and explainability by MQM indicate better preservation of meta-evaluation quality. Markers show the performance of the six synthesis-based meta-evaluation setups from~\citet{shayegh-etal-2025-feeding}, placed at their synthesized datasets' respective $\beta_0$. All comparisons are based on SPA.}
\label{fig:3}
\vspace{-5pt}
\end{figure*}
...
Figures~\ref{fig:3} and~\ref{fig:results_all}\footnote{Results for some datasets are deferred to the Appendix~\ref{app:all_datasets} due to space constraints.} show our method's performance across datasets over a $\beta$ sweep spanning $[\beta_{\min}, \beta_{\max}]$, plus explicitly every $\beta_{kk'}$ in the dataset. The monotonically increasing adequacy-over-fluency curves confirm that our reweighting mechanism effectively controls this balance. Furthermore, within a range around $\beta_0$ where ESS remains high, our method successfully preserves the original meta-evaluation's MQM adherence and explainability by MQM.

The strong correlation between ESS and both MQM adherence and explainability by MQM confirms ESS as a robust reliability metric. Practically, ESS identifies the reachable range for controlling the adequacy--fluency balance without degrading meta-evaluation quality.

The permissible deviation from $\beta_0$ is fundamentally constrained by the dataset itself. In datasets like jazh~24, most $\beta_{kk'}$ values concentrate within a narrow band, limiting reweighting capacity. Consequently, if a dataset lacks systems that intrinsically support a fluency-heavy evaluation, this imposes a strict upper limit on the achievable balance. 

Compared to \citet{shayegh-etal-2025-feeding}, our method consistently yields superior MQM adherence, explainability by MQM, or both across all datasets, whether controlled by $\beta$ or adequacy-over-fluency score. The only exceptions occur at extreme adequacy-over-fluency values (e.g., in zhen~23, ende~22, and ende~24) which their synthetic method reaches but our reweighting cannot. However, explainability by MQM drops precipitously for both methods in these regimes, rendering those extremes unreliable and practically infeasible anyways. Overall, our method strictly improves upon \citet{shayegh-etal-2025-feeding} in preserving the faithfulness of the meta-evaluation.

Analyzing the markers in Figure~\ref{fig:3} reveals a critical limitation of synthetic systems. In \citet{shayegh-etal-2025-feeding}'s setups that retain real systems, explainability by MQM is preserved but MQM adherence degrades. Conversely, in their fully synthesized pools, MQM adherence is preserved while explainability by MQM collapses. Explainability by MQM measures the meta-evaluation's response to scorer noise while preserving natural biases; it is informed, but not strictly dictated, by MQM. While fully synthetic pools accurately measure MQM adherence (because it is the only remaining signal), they fail to rank real scorers reliably, confirming our hypothesis that synthetic systems are unrepresentative. This failure stems from their construction: segments are regrouped to maximize between-system variance. By the law of total variance~\cite{soch2025statproofbook}, since total pool variance is fixed, altering between-system variance inevitably alters within-system variance. This yields systems with unrealistically low score variances, being unrepresentative and also distorting SPA's confidence estimations.
...
\caption{Ablation study of our algorithm on the ende24 dataset ($K=16$). We sweep $\hat\beta$ across 50 values in each of two distinct regimes. \# Supports (\# Candidates) indicates the number of supports (candidates) visited prior to termination.}
\end{table*}
...
\subsection{Efficiency and Ablation Analysis}\label{sec:comp}

We evaluate the computational efficiency of Algorithm~\ref{alg:reweight} and ablate its three pruning mechanisms using the ende24 dataset ($K=16$). We sweep $\hat\beta$ across 50 values in two regimes: a narrow band around the natural balance $\beta_0$, and the full reachable range $[\beta_{\min}, \beta_{\max}]$. The former represents the practical use case, as performance is more reliable here (\S\ref{sec:results}) and minor adjustments reflect realistic demands. The latter serves as an extreme regime to stress-test the algorithm. We compare against a numerical optimization baseline using varying grid resolutions $N$ and a fixed tolerance of $10^{-3}$. Tables~\ref{tab:solve-exact-operation-counts} and~\ref{tab:solve-exact-theorem-invocations} summarize the results.
...
Reaching extreme $\hat\beta$ targets requires sparse weightings. In this regime, Theorem~\ref{thm:admissible} (admissible supports) drives the pruning, triggering over 65.5K times (Table~\ref{tab:solve-exact-theorem-invocations}) to safely skip infeasible branches. The exact algorithm demands more computation than the numerical baseline across the full range, which is the cost of guaranteeing the global minimum. The ablation confirms that combining all pruning theorems reduces the full-range computational cost by more than $3\times$ compared to the unpruned exact search.


\subsection{Weighted Meta-Evaluation}

Having validated our reweighting mechanism, we now apply it to compare variants of the popular scorers MetricX~\citep{juraska-etal-2024-metricx} and xCOMET~\citep{10.1162/tacl_a_00683} across a sweep of $\beta$. Figure~\ref{fig:metrics} reports the weighted soft pairwise accuracy (SPA) for the zhen23 and ende24 datasets.
...
\end{center}
\vspace{-5pt}
\caption{Weighted meta-evaluation performance (SPA$_\beta$) of MetricX and xCOMET variants as a function of the adequacy--fluency balance parameter $\beta$. In zhen23 (left), scorer rankings invert depending on the target balance. In ende24 (right), relative performance remains stable across the reachable range. $\beta$ interval is bounded by $\mathrm{ESS} \ge K-1$, where $K$ is the total number of systems ($15$ for zhen23, $16$ for ende24).}
\label{fig:metrics}
\vspace{-5pt}
\end{figure*}
...
Machine translation meta-evaluation inherits the adequacy–fluency tradeoff, with its balance dictated by the specific combination of translation systems in the evaluation data. Because this system set is a small, unrepresentative sample that fluctuates across datasets, the resulting adequacy–fluency balance is uncontrolled and subject to change.
...
To validate meta-evaluation faithfulness, we introduced a scorer-augmentation framework that establishes a ground-truth relative identity for real metrics. Our experiments demonstrated that our reweighting method successfully controls the adequacy–fluency balance and preserves evaluation integrity, outperforming system-synthesis approaches. Finally, our analysis of popular scorers showed that while metric rankings are sensitive to~$\beta$, controlling this parameter does not resolve the broader instability inherent to system selection.

\section*{Limitations}

Our reweighting mechanism is strictly bounded by the intrinsic diversity of the underlying dataset. If a dataset lacks systems that exhibit isolated adequacy or fluency errors (e.g., jazh~24), the reachable range of $\beta$ is narrow. Forcing $\beta$ beyond this range requires sparse weights, which reduces the effective sample size (ESS) and compromises meta-evaluation reliability. Further expanding the admissible range for those datasets with limited flexibility remains a direction for future work.

Moreover, controlling the adequacy--fluency balance does not resolve the broader instability of meta-evaluation. As demonstrated in \S\ref{sec:loo}, scorer rankings remain sensitive to system selection even when $\beta$ is fixed. Future work must identify the root causes of system-selection instability. We would like to investigate other axes of variation across translation systems that disrupt scorer rankings.
...
By \ref{eq:b} and Proposition~\ref{lem:pairwise-gap}:
\begin{align}
...
By definition of $\beta$, we have have $0 < \hat\beta < 1$. Therefore,, this difference provides one strictly positive and one strictly negative direction. Consequently, $M$ is indefinite with rank two, possessing exactly two nonzero eigenvalues:
\begin{align}
...
For every other coordinate $m \neq j$, the coefficient $(1 + 2\lambda_j \theta_m) = 1 - \frac{\theta_m}{\theta_j}$ is strictly non-zero: $\theta_m{=}0$ for flat directions $m{\ge}3$ and $\frac{\theta_{m}}{\theta_j}{<}0$ for $m{\in}\{1, 2\}$ by~\eqref{eq:eigenvalues}. Therefore, every $u_m:m \neq j$ is uniquely fixed to
\begin{align}
...
\begin{align}
    \theta_j &u_j(\eta)^2 + {r}_j u_j(\eta) \\&= \theta_j \left(\frac{-\eta {r}_j}{1 + 2\eta\theta_j}\right)^2 + {r}_j \left(\frac{-\eta {r}_j}{1 + 2\eta\theta_j}\right) \\
...
\paragraph{[Part (b)]}
By assumption, $w^\star$ is a global optimum of~\eqref{problem} with support $S=S(w^\star)$. Because $w^\star_k > 0$ for all $k \in S$, the non-negativity constraints are strictly inactive. $w^\star$ locally minimizes $\tfrac12\|w\|^2$ subject to the two equality constraints $\mathbf 1^\top w = 1$ and $b(w,\hat\beta)=0$ on~$S$. If the constraint gradients are linearly independent on $S$, the linear independence constraint qualification holds, ensuring $w^\star$ is a stationary point of~\eqref{eq:probinu} and thus covered by the enumeration of Part~(a). Otherwise,
the constraint gradient $\nabla_w b(w^\star, \hat\beta)$ is a scalar multiple of $\mathbf 1$. In the reduced coordinate space~(\ref{eq:affinetransform}--\ref{eq:tildeb}), the chain rule gives the reduced gradient:
\begin{align}
    \nabla_u \tilde{b}(u^\star, \hat\beta) &= \nabla_u {b}(u_0 + Nu^\star, \hat\beta)\notag\\
    &= N^\top \nabla_w b(w^\star, \hat\beta) = 0\label{eq:redgradzero}
\end{align}
Thus, the reduced gradient vanishes at $u^\star$. 

Together with feasibility $\tilde{b}(u^\star, \hat\beta)=0$, we expand the quadratic function~\eqref{eq:tildeb} around $u^\star$ for a step with size $t$ and direction $\delta$:
\begin{align}
    &\tilde{b}(u^\star + t\delta, \hat\beta)\notag \\
    &=\underbrace{\tilde{b}(u^\star, \hat\beta)}_{=0} + \underbrace{t \nabla_u \tilde{b}(u^\star, \hat\beta)^\top \delta}_{= 0} + \tfrac{t^2}{2} \delta^\top \underbrace{\nabla^2_u \tilde{b}(u^\star, \hat\beta)}_{= 2M} \delta \notag\\
    &= t^2\delta^\top M \delta
\end{align}
Thus, every direction $\delta$ where $\delta^\top M \delta = 0$ generates a line $u(t) = u^\star + t\delta$ that remains entirely feasible for all $t \in \mathbb{R}$. Because $u^\star$ minimizes the objective $\frac{1}{2}\|u\|^2$ over this fully feasible line, the objective restricted to the line,
\begin{align}
    \|u^\star + t\delta\|^2 = \|u^\star\|^2 + 2t(u^{\star\top}\delta) + t^2\|\delta\|^2
\end{align}
must achieve a minimum at $t = 0$. This asserts
\begin{align}
    u^{\star\top}\delta = 0
\end{align}
for all such null directions.

By Theorem~\ref{thm:admissible}, the optimum's support $S$ must be admissible, so it falls under Case~1 or Case~2 of Part~(a). If $S$ is degenerate (Case~1), then $M=0$: the entire face is feasible and $u^\star=0$ is the unique minimizer of $\tfrac12\|u\|^2$ over it. Otherwise (Case~2), $M$ is indefinite of rank two by~\eqref{eq:eigenvalues}, so its null directions span the reduced space: they contain the kernel directions $\{v_j\}_{j\ge3}$ together with $\sqrt{-\theta_2}\,v_1 \pm \sqrt{\theta_1}\,v_2$, whose span is all of $\mathbb{R}^{|S|-1}$. Hence $u^{\star\top}\delta = 0$ for a spanning set, forcing $u^\star = 0$, and then $r = -2Mu^\star = 0$ by~\eqref{eq:redgradzero} and $c = \tilde b(0,\hat\beta) = 0$. In all scenarios, $w^\star$ is the uniform weighting on $S$, which Part~(a) enumerates in either case: as the unique stationary point when $S$ is degenerate, and as the stationary point at $\eta=0$ when $c=0$ otherwise. The global optimum is therefore definitively contained within the enumerated candidate set in (a).
\end{proof}
...
\begin{proof}
The balance constraint $b(w, \hat\beta) = 0$ is governed by an indefinite quadratic matrix $H = \hat\beta ff^\top - (1-\hat\beta)aa^\top$, according to~\eqref{eq:binhg} in the proof of Theorem~\ref{thm:candidates}. This makes the constraint domain is non-convex.
...
\end{align}
where $H$ is the indefinite matrix $H = \hat\beta ff^\top - (1-\hat\beta)aa^\top$, following to~\eqref{eq:binhg}.
...
$w$ satisfies KKT conditions across both active and inactive supports via (i) and (ii), and the second-order subspace sufficiency condition via (iii), $w$ is certified to be a global optimum of Problem~\eqref{problem}.
\end{proof}

\section{Results for All Datasets}
...
\begin{tabular}{@{}c@{}}
    \quad ende 22 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__ende22_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad zhen 21 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__zhen21_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad enes 24 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__enes24_DiagTJ_synth25_nolegend.pdf}
\end{tabular}
...
\begin{tabular}{@{}c@{}}
    \quad heen 23 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__heen23_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad jazh 24 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__jazh24_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad ted ende \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__ted_ende_DiagTJ_synth25_nolegend.pdf}
\end{tabular}
...
\begin{tabular}{@{}c@{}}
    \quad ted zhen \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__ted_zhen_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad zhen 22 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__zhen22_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad zhen 23 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__zhen23_DiagTJ_synth25_nolegend.pdf}
\end{tabular}
...
\begin{tabular}{@{}c@{}}
    \quad ende 21 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__ende21_DiagTJ_synth25_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad ende 24 \\
    \includegraphics[height=5cm]{figures/preference_spa/orientation__ende24_DiagTJ_synth25.pdf}
\end{tabular}

\caption{Main results for all datasets. Layout follows Figure~\ref{fig:3}.}
\label{fig:results_all}
\end{figure*}

\section{Results for Pearson Correlation}
\label{app:pearson}
...
\begin{tabular}{@{}c@{}}
    \quad ende 22 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_ende22_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad zhen 21 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_zhen21_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad enes 24 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_enes24_DiagTJ_nolegend.pdf}
\end{tabular}
...
\begin{tabular}{@{}c@{}}
    \quad heen 23 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_heen23_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad jazh 24 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_jazh24_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad ted ende \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_ted_ende_DiagTJ_nolegend.pdf}
\end{tabular}
...
\begin{tabular}{@{}c@{}}
    \quad ted zhen \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_ted_zhen_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad zhen 22 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_zhen22_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad zhen 23 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_zhen23_DiagTJ_nolegend.pdf}
\end{tabular}
...
\begin{tabular}{@{}c@{}}
    \quad ende 21 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_ende21_DiagTJ_nolegend.pdf}
\end{tabular}\hfill
\begin{tabular}{@{}c@{}}
    \quad ende 24 \\
    \includegraphics[height=5cm]{figures/preference_pearson/orientation__pearson_ende24_DiagTJ.pdf}
\end{tabular}
