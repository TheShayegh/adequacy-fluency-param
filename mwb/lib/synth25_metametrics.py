...
and SCORER synthesis must never be confused):
  - This module is entirely self-contained. It is NOT wired into
    mwb.lib.metametrics.METAMETRICS/WEIGHTED_METAMETRICS or
    mwb.lib.consistency -- no existing caller (scorer_scores, weighted_
  ...
  - A coarser implementation of the same synthesis rule once existed
    ...
    alpha-range extender; it has since been removed as unused. This module
...
mwb.lib.spa_plane.positional_af_matrices can validate that Adequacy/Fluency
...
from mwb.lib.alpha import alpha_0 as _alpha_0
...
# Same floor used throughout this project (mwb.lib.consistency,
# mwb.lib.spa_plane) before trusting SPA's permutation p-values / a
...
DEFAULT_SEED = 4          # SPA's permutation-test null-sampling seed --
                           # matches mwb.lib.spa_plane.DEFAULT_SEED so a
...
def synth25_value(
  ...
  a mwb.lib.synthetic_scorers dial family, which is how this function is
  reused to compare SYNTHETIC scorers against the synth25 baseline
  (kept out of THIS module, per its isolation policy -- see
  mwb.lib.synth25_orientation, the one place that bridges the two).
...
def pool_alpha_0(
  ...
  """alpha_0 (mwb.lib.alpha.alpha_0: Var(a)/(Var(a)+Var(b)) at uniform
  weight) of one pool's OWN system-level Adequacy/Fluency MQM -- a property
  of the pool's composition alone, independent of any scorer/donor/
  metametric. a_pos/b_pos: already (K, n_pos) row-aligned to the K real
  ...
  further restrict to any one donor's coverage, since this is a
  dataset-level constant, not a per-donor one)."""
  rng = np.random.default_rng(tie_seed)
  order_adequacy = _segment_rank_order(a_pos, rng)
  order_fluency = _segment_rank_order(b_pos, rng)
...
def synth25_scorer_scores(
  ...
  screened roster, mwb.lib.consistency.real_systems); these K real systems are
