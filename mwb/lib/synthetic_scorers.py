"""Synthetic scorer construction: given a real donor scorer's segment
scores, builds two families of synthetic scorers per donor -- one dialed on
Adequacy MQM (dial A), one on Fluency MQM (dial B) -- each a straight-line
sweep between the donor itself (dial=0) and some aspect-determined endpoint
...
donor_family_additive_mean's 'T' (AllMQM) aspect; the adequacy-fluency
...
Reuses mwb.lib.spa_plane's positional alignment (a_pos, b_pos, and a donor's own
...
# reordering needed, unlike mwb.lib.synthetic_scorer_orientation.
...
# orientation_vs_alpha.py's --dial-step/--dial-n.
def make_abt_dial_grid(step: float = 0.005, n: int = 201) -> tuple[float, ...]:
  return tuple(round(-1.5 + 3.0 ** (step * k), 10) for k in range(n))


def make_abtj_j_dial_grid(step: float = 0.005, n: int = 201) -> tuple[float, ...]:
  return tuple(round(-3.5 + 4.0 ** (step * k), 10) for k in range(n))


ABT_DIAL_GRID = make_abt_dial_grid()
ABTJ_J_DIAL_GRID = make_abtj_j_dial_grid()
...
# dial=0-reproduces-the-donor convention. Shared with mwb.lib.
...
def donor_family_spa_points(
  ...
  two axes mwb.lib.spa_plane.scorer_spa_points uses. donor_seg/a_pos/b_pos must
  ...
  (the official All-MQM total, mwb.lib.metric_scores.load_human_seg_scores,
  ...
  All MQM instead of either aspect (mirrors mwb.lib.synthetic_scorer_alpha_grid.
...
def all_synthetic_family_points(
  ...
  positions with a_pos/b_pos -- the synthetic-scorer analogue of mwb.lib.
