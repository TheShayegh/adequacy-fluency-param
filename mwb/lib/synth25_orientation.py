...
allmqm_orientation(SPA_synth25): mwb.lib.synthetic_scorer_orientation's
...
system-synthesis method) instead of the weighted-SPA(alpha)/weighted-
PA(alpha) family compute_scorer_orientation_vs_alpha.py sweeps. This
...
Same dial families (mwb.lib.synthetic_scorers' A/B/T sweep for one donor
scorer), same reduction (mwb.lib.synthetic_scorer_orientation.orientation_score:
...
each dial. Since SPA_synth25/PA_synth25 take no alpha argument (PA/SPA
computed once, on the fixed Row-7 union pool), each donor's A/B/T family
collapses to a single orientation_score per family rather than a curve
over alpha. Averaged over donors, this gives 3 dataset-level CONSTANTS --
meant to be drawn as horizontal reference lines on an existing
orientation-vs-alpha plot (they do not depend on alpha at all), not as new
points on the alpha axis.
...
mwb.lib.synthetic_scorer_alpha_grid, an unrelated axis -- see mwb.lib.synth25_
...
from mwb.lib.synthetic_scorer_alpha_grid import ASPECT_DONORS, _aspect_donor_seg
...
def orientation_tag(
  ...
  mwb.lib.synthetic_scorer_orientation.orientation_tag's abbreviated suffix)
  ...
  (mwb.lib.synthetic_scorer_orientation's _synthesis_suffix/_dial_preset_suffix);
