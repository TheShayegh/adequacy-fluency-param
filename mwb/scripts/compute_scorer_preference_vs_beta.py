...
(mwb.lib.synthetic_scorer_preference) for every real base scorer s_0 in a
...
preference family, mwb.lib.synthetic_scorers.base_scorer_family_additive_mean_af,
...
spa|pa|pearson; mwb.lib.synthetic_scorer_beta_grid.base_scorer_beta_dial_grid)
...
see mwb.lib.synthetic_scorers.base_scorer_family_adequacy_adherence).

Usage: python -m mwb.scripts.compute_scorer_preference_vs_beta [--dataset ende21]
...
(mwb.lib.synthetic_scorers.make_aft_dial_grid/make_aftj_j_dial_grid) --
...
EXTRA base_scorer_beta_dial_grid(..., synthesis='offset', dial_grid=mwb.lib.
...
from mwb.lib.beta import beta_0, beta_min_max, union_beta_grid, union_beta_grid_beta_std
from mwb.lib.consistency import real_scorers, real_systems
from mwb.lib.reweighted_consistency import solve_w_for_datasets
from mwb.lib.synthetic_scorer_beta_grid import ASPECT_BASE_SCORERS, beta_grid_around, base_scorer_beta_dial_grid
from mwb.lib.synthetic_scorer_preference import (
    DIAL_GRID, NEG_J_DIAL_GRID, base_scorer_preference_by_beta, full_range_betas, preference_tag, save_preference_data,
    union_grid_tag,
)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--union-grid', action=argparse.BooleanOptionalAction, default=True,
                       help='use the union of every pairwise beta_ij of D with a plain uniform '
                            '[beta_min, beta_max] sweep at --step (mwb.lib.beta.union_beta_grid) instead '
  ...
  parser.add_argument('--beta-std-grid', action=argparse.BooleanOptionalAction, default=False,
                       help='like --union-grid (the beta_ij union stays), but the uniform-sweep half is '
                            'evenly spaced in BETA_STD space instead of beta space (mwb.lib.beta.'
  ...
  parser.add_argument('--aspect-base-scorers', action=argparse.BooleanOptionalAction, default=False,
                       help="compute only mwb.lib.synthetic_scorer_beta_grid.ASPECT_BASE_SCORERS ('AdequacyMQM', "
  ...
  parser.add_argument('--workers', type=int, default=1,
                       help='parallel worker processes (ProcessPoolExecutor), applied to BOTH expensive '
                            'steps: the solve_w_for_datasets beta-solving pass (mwb.lib.reweighted_consistency, '
